...
| No `phpcs:disable WordPress.Security.*` in `includes/` or the main file; echo kit markup through one delegated `$render()`/`kit()` point and use per-line `phpcs:ignore` with a reason | Reviewers flag file-wide security disables as escaping/nonce failures (hit this plugin 2026-09-06 and perxel-ai-translate 2026-09-22); `bin/check-suppressions.sh` (run by `composer run lint`, so CI) enforces it |
